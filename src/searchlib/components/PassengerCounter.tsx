  ...
  return (
    <div className="w-full">
      <div className={clsx(styles[variant], styles.container)}>
        ...
        >
          {error ?? (lang === "es" ? "Pasajeros" : "Passengers")}
        </span>
        <input
          type="number"
          ...
          value={passengers ?? 0}
          onChange={(e) =>
            setter("passengers", parseInt(e.currentTarget.value))
          }
          className={clsx(styles[variant], styles.input)}
        />
        <button
          onClick={() => alterPassegnerCount(-1)}
          className={`group ${styles.btn} ${styles.iconContainer}`}
        >
          <HeroiconsMinus20Solid
            className="h-5 w-5 text-gray-400 duration-200 group-hover:text-primary-600"
            aria-hidden="true"
          />
        </button>
        <button
